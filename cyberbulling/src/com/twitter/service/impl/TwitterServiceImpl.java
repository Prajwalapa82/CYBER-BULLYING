...
public class TwitterServiceImpl implements TwitterServiceIF {
	...
	public StatusInfo retriveTweetsAndStore() {
		...
		try {
			...
			Twitter twitter = new TwitterFactory().getInstance();
			twitter.setOAuthConsumer("GXvmth0FawnOZS1g8JlrWK54Q",
					"wnR1pVIsOaPnLzJZLi2cYUD505SHkHp7AzXHIJGYxlIJzi468A");
			twitter.setOAuthAccessToken(new AccessToken(
	...
	public StatusInfo classifyTweet() {
		...
		try {
			...
			List<ProbabilityInfo> contigencyInfos = twitterDoa
					.retriveAllProbability();

			if (null == contigencyInfos || contigencyInfos.isEmpty()) {
				statusInfo = new StatusInfo();
				statusInfo
						.setErrMsg(TwitterConstants.Message.PLEASE_PERFORM_PROBABILITY);
				statusInfo.setStatus(false);
				return statusInfo;
			}

			// Obtain Distinct Tweets from probability Table
			List<Integer> distinctTweetIds = twitterDoa
					.retriveDistinctTweetIdsFromProbability();

			if (null == distinctTweetIds || distinctTweetIds.isEmpty()) {
				statusInfo = new StatusInfo();
				statusInfo
						.setErrMsg(TwitterConstants.Message.NO_TWEETS_PROBABILITY);
				statusInfo.setStatus(false);
				return statusInfo;
			}
